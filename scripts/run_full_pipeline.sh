...
if is_true "${DO_SLICE:-true}"; then
  ...
  COMMAND=(
    ...
    --aligner-num-workers "${ALIGNER_NUM_WORKERS:-1}"
    --source-concurrency "${SOURCE_CONCURRENCY:-1}"
    ...
    --preprocess-max-chunk-sec "${PREPROCESS_MAX_CHUNK_SEC:-15}"
    --rms-silence-frame-ms "${RMS_SILENCE_FRAME_MS:-25}"
    --rms-silence-hop-ms "${RMS_SILENCE_HOP_MS:-5}"
    --rms-silence-percentile "${RMS_SILENCE_PERCENTILE:-25}"
    --rms-silence-threshold-multiplier "${RMS_SILENCE_THRESHOLD_MULTIPLIER:-1.8}"
