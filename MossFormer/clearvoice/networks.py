...
class SpeechModel:
    ...
    def process(self, input_path, online_write=False, output_path=None):
        ...
        if self.args.task == 'target_speaker_extraction':
            ...
            def process_loaded_sample(sample):
                input_audio, wav_id, input_len, scalars, audio_info = sample
                self.data = {}
            ...
            preprocess_workers = max(1, int(getattr(self.args, 'preprocess_workers', 1)))
            with torch.no_grad():
                if preprocess_workers == 1:
                    for idx in tqdm(range(num_samples)):  # Loop over all audio samples
                        process_loaded_sample(data_reader[idx])
                else:
                    max_pending = min(num_samples, preprocess_workers * 2)
                    next_idx = 0
                    future_to_idx = {}
                    ...
                    def submit_next(executor):
                        ...
                        if next_idx >= num_samples:
                            return False
                        future = executor.submit(data_reader.__getitem__, next_idx)
                        future_to_idx[future] = next_idx
                        next_idx += 1
                        return True

                    with ThreadPoolExecutor(max_workers=preprocess_workers) as executor:
                        while len(future_to_idx) < max_pending and submit_next(executor):
                            pass
                        ...
                        with tqdm(total=num_samples) as progress:
                            while future_to_idx:
                                ...
                                for future in done:
                                    ...
                                    while len(future_to_idx) < max_pending and submit_next(executor):
                                        pass
                                    process_loaded_sample(future.result())
                                    progress.update(1)
