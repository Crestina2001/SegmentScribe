...
def _compute_mossformer2_fbank_features(audio_segment, args):
    """Compute fbank + delta + delta-delta features for one audio segment."""
    ...
    return torch.cat([fbanks, fbank_delta, fbank_delta_delta], dim=1)


def _decode_mossformer2_segment_batch(model, device, audio_segments, args):
    """Decode fixed-length MossFormer2 windows as one GPU batch."""
    fbanks = [
        _compute_mossformer2_fbank_features(audio_segment, args)
        for audio_segment in audio_segments
    ]
    fbanks = torch.stack(fbanks, dim=0).to(device)
    audio_batch = torch.stack(audio_segments, dim=0).to(device)

    with torch.inference_mode():
        pred_mask = model(fbanks)[-1]

    spectrum = stft(audio_batch, args)
    pred_mask = pred_mask.permute(0, 2, 1).unsqueeze(-1)
    masked_spec = spectrum * pred_mask.detach()
    masked_spec_complex = masked_spec[..., 0] + 1j * masked_spec[..., 1]
    return istft(masked_spec_complex, args, audio_batch.shape[-1]).cpu()
...
def decode_one_audio_mossformer2_se_48k(model, device, inputs, args):
    ...
    if input_len > args.sampling_rate * args.one_time_decode_length:  # 20 seconds
        ...
        if online_decoding:
            ...
            inference_batch_size = max(1, int(getattr(args, 'inference_batch_size', 1)))

            # Process audio in sliding window segments
            while current_idx + window <= t:
                batch_segments = []
                batch_indices = []
                while current_idx + window <= t and len(batch_segments) < inference_batch_size:
                ...
                output_segments = _decode_mossformer2_segment_batch(
                    model,
                    device,
                    batch_segments,
                    args,
                ...
                for output_segment, segment_idx in zip(output_segments, batch_indices):
                    if segment_idx == 0:
                        outputs[segment_idx:segment_idx + window - give_up_length] = output_segment[:-give_up_length]
                    else:
                        output_segment = output_segment[-window:]
                        outputs[segment_idx + give_up_length:segment_idx + window - give_up_length] = output_segment[give_up_length:-give_up_length]
        ...
        # Pass filter banks through the model without autograd overhead.
        with torch.inference_mode():
            Out_List = model(fbanks)
        pred_mask = Out_List[-1]  # Get the predicted mask
        audio_device = audio.to(device)
        spectrum = stft(audio_device, args)  # Apply STFT to the audio
        pred_mask = pred_mask.permute(2, 1, 0)  # Permute dimensions for masking
        masked_spec = spectrum * pred_mask.detach()  # Apply mask to the spectrum
